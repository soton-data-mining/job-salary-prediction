...
class NeuralNetRegressor(BaseModel):
    def ml_model(self):
        ...
        model = import_model('ee')

        model = Sequential()
        model.add(Dense(100, input_dim=52, activation='relu',
                        kernel_initializer=initializers.RandomNormal(
                                mean=5, stddev=4, seed=None)))
        model.add(Dense(1, activation='linear',
                        kernel_initializer=initializers.RandomNormal(
                                mean=1, stddev=0.9, seed=None)))

        #adam = opt.Adam(lr=0.003, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
        ...
        model.compile(loss='mean_absolute_error', optimizer=rms, metrics=[metrics.mae])
        # optimizer='adam'
        ...
        export_model(model, 'ff')
        """
        a = model.predict(self.x_test)

        print(a)
        BaseModel.export_prediction(a,'asd')
