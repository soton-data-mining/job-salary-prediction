...
def get_feature_word_count(column_to_count):
    ...
    for data in column_to_count.iterrows():
        ...
        data_digits_removed = ''.join([i for i in string_data if not i.isdigit()])# Remove digits
        
        puct_exclude = set(string.punctuation)
        data_puctuations_removed  = ''.join(ch for ch in data_digits_removed if ch not in puct_exclude)# Remove punctuations
        data_puctuations_removed = data_puctuations_removed.lower()# Lowercase the string
        
        data_as_list = data_puctuations_removed.split()# Turn into list
